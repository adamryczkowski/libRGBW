...
namespace libRGBW {
    ...
    RGBW WhiteConverter::getRGBW(RGB rgb, float amplify) {
        ...
        RGBW result;
        // Serial<<"ile_jest_bialego_w: ("<<ile_jest_bialego_w.R<<", "<<ile_jest_bialego_w.G<<", "<<ile_jest_bialego_w.B<<")\n";
...
//	RGB RGB_from_RGBW(RGBW kolor, float& intensywnosc) {
    ...
    float WhiteConverter::calculateMaxAmplification(RGB rgb) {
        ...
        if (rgbw.W == maxC) {
            ...
            if (rgb.R > 0)
                tmp.R = res.R /
                        rgb.R; //ile razy możemy dodatkowo zwiększyć kanał R, zanim wykorzystamy 100% mocy tego kanału?
            else
            ...
            float max_brightness = min(tmp.R, min(tmp.G,
                                                  tmp.B));//jaką część światła o kolorze rgb można dodać używając rgb. Jak pomnożymy przez intensywność naszego obecnego światła, to dostaniemy wartość w jednostce natężenia.
    ...
    float PerceptualConverter::getIntensity(perceptualRGB pcolor) {
        return (sqrt((pcolor.pR * pcolor.pR) + (pcolor.pG * pcolor.pG) + (pcolor.pB * pcolor.pB)) *
                0.577350269); // 0.577350269=1/sqrt(3)
    }
